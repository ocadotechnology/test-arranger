---
name: tester
description: Android test creation guidelines and conventions; use when user asks to create new tests or update existing ones
---
...
## Android-specific critical rules

⚠️ **These rules are non-negotiable for Android tests:**

-   **EVERY custom arranger must be registered in `arranger.properties`** under the `arranger.android.customArrangers` property.
-   After creating a new `CustomArranger`, immediately add its fully qualified class name to `arranger.properties`.
-   Verify `arranger.properties` exists in `src/test/resources/` (unit tests) or `src/androidTest/resources/` (instrumented tests).
-   If tests behave unexpectedly or custom arrangers appear to be ignored, check that all custom arrangers are listed in `arranger.android.customArrangers`.
-   When reviewing or debugging test failures, always verify custom arranger registration first—this is often the root cause of confusing test behavior in Android.